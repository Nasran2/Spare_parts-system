...
<div class="max-w-xl">
    <div class="bg-white rounded-xl shadow p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">New Expense Category</h3>
        <form action="{{ route('expense-categories.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Expense Limit (0 for no limit)</label>
                <input type="number" step="0.01" name="limit" value="{{ old('limit', 0) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('limit') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="reset_frequency" id="reset_frequency" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        <option value="lifetime" {{ old('reset_frequency') == 'lifetime' ? 'selected' : '' }}>Lifetime</option>
                        <option value="monthly" {{ old('reset_frequency') == 'monthly' ? 'selected' : '' }}>Monthly</option>
                    </select>
                </div>
                <div id="reset_date_wrapper" class="{{ old('reset_frequency') == 'monthly' ? '' : 'hidden' }}">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Reset Date (Day of Month)</label>
                    <input type="number" min="1" max="31" name="reset_date" value="{{ old('reset_date', 1) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t">
                ...
                <button type="submit" class="px-6 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800">Save Category</button>
            </div>
        </form>
    </div>
</div>
