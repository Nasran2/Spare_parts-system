...
@section('page-title', 'Make Payment - ' . $customer->name)

@section('content')
<div class="space-y-6">
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-xl font-bold text-gray-800">Payment Allocation for {{ $customer->name }}</h2>
            <p class="text-gray-600 text-sm mt-1">Total Due: <span class="font-bold text-red-600">{{ \App\Models\Setting::get('currency_symbol', 'Rs') }} <span id="total-due-display">{{ number_format($unpaidSales->sum('due_amount') + $openingBalanceDue, 2, '.', '') }}</span></span></p>
        </div>

        <form action="{{ route('customers.bulk-payment.store', $customer->id) }}" method="POST" class="p-6" onsubmit="return validateAllocations()">
            @csrf
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-200" id="bills-table-body">
                        ...
                        @foreach($unpaidSales as $sale)
                            <tr class="hover:bg-gray-50" data-due="{{ $sale->due_amount }}">
                                <td class="px-4 py-3 text-gray-600">{{ $sale->sale_date ? \Carbon\Carbon::parse($sale->sale_date)->format('Y-m-d') : $sale->created_at->format('Y-m-d') }}</td>
                                <td class="px-4 py-3 font-mono text-gray-800">{{ $sale->sale_no }}</td>
                                <td class="px-4 py-3 text-right text-gray-600">{{ number_format($sale->total_amount, 2) }}</td>
                                <td class="px-4 py-3 text-right font-bold text-red-600">{{ number_format($sale->due_amount, 2) }}</td>
                                <td class="px-4 py-3 text-right">
                                    <input type="hidden" name="allocations[{{ $allocIndex }}][type]" value="sale">
                                    <input type="hidden" name="allocations[{{ $allocIndex }}][id]" value="{{ $sale->id }}">
                                    <input type="number" step="0.01" name="allocations[{{ $allocIndex }}][amount]" class="allocation-input w-32 px-3 py-1 text-right border rounded focus:border-blue-500 focus:ring focus:ring-blue-100" value="0" min="0" max="{{ $sale->due_amount }}" onchange="updateTotalApplied()">
                                </td>
                            </tr>
                            @php $allocIndex++; @endphp
                        @endforeach
                        
                        @if($unpaidSales->isEmpty() && $openingBalanceDue <= 0)
                            <tr>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                ...
                @else
                    <a href="{{ route('customers.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">Cancel</a>
                @endif
                <button type="submit" class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold transition shadow" id="submit-btn" {{ ($unpaidSales->isEmpty() && $openingBalanceDue <= 0) ? 'disabled' : '' }}>
