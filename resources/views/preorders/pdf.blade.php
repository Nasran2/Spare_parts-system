...
body{font-family:DejaVu Sans,Arial,sans-serif;font-size:11px;color:{{ $pdfSettings['text_color'] ?? '#1f2937' }}}.header{border-bottom:2px solid {{ $pdfSettings['line_color'] ?? '#1d4ed8' }};padding-bottom:10px;margin-bottom:12px}
@php
    $logoRadius = '0';
    if (($pdfSettings['logo_shape'] ?? 'original') === 'round') $logoRadius = '50%';
    elseif (($pdfSettings['logo_shape'] ?? 'original') === 'box') $logoRadius = '10px';
@endphp
...
@php $isExclHidden = $preOrder->pdf_tax_display === 'exclusive_hidden'; $tm = $isExclHidden ? (1 + ((float)$preOrder->custom_tax_rate / 100)) : 1; $dispSubtotal = (float)$preOrder->subtotal * $tm; $dispOrderDiscount = (float)$preOrder->discount_amount * $tm; @endphp
<table class="totals"><tr><td>Subtotal</td><td class="num">{{ $currency }}{{ number_format($dispSubtotal,2) }}</td></tr><tr><td>Discount</td><td class="num">{{ $currency }}{{ number_format($dispOrderDiscount,2) }}</td></tr>@if(!in_array($preOrder->pdf_tax_display, ['inclusive', 'exclusive_hidden']))<tr><td>Tax</td><td class="num">{{ $currency }}{{ number_format((float)$preOrder->tax_amount,2) }}</td></tr>@endif<tr class="grand"><td>Grand Total</td><td class="num">{{ $currency }}{{ number_format((float)$preOrder->grand_total,2) }}</td></tr>@if($kind==='invoice')<tr><td>Paid</td><td class="num">{{ $currency }}{{ number_format((float)$preOrder->paid_amount,2) }}</td></tr><tr><td>Pending Cheques</td><td class="num">{{ $currency }}{{ number_format((float)$preOrder->held_cheque_amount,2) }}</td></tr><tr><td>Due</td><td class="num">{{ $currency }}{{ number_format((float)$preOrder->due_amount,2) }}</td></tr><tr><td>Payment Status</td><td class="num">{{ $preOrder->payment_status === 'partial' ? 'Partially Paid' : ucfirst($preOrder->payment_status) }}</td></tr>@endif</table>
