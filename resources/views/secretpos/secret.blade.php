...
<body class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-white p-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow">
                <div class="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
                     <div class="flex items-center text-blue-600">
                        ...
                        <h3 class="text-lg font-bold brand-font uppercase tracking-wider text-slate-900">Hidden Ranges</h3>
                    </div>
                    <button type="button" onclick="addRange()" class="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-lg font-bold uppercase tracking-wider transition-colors shadow">
                ...
                <div id="ranges" class="space-y-3">
                    @foreach($config['hidden_ranges'] as $idx => $r)
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-center bg-slate-50 p-3 rounded-lg border border-slate-200 hover:border-slate-300 transition-colors group">
                            <div class="md:col-span-4">
                                <label class="block text-[10px] text-slate-500 uppercase tracking-wider mb-1 px-1">Min Amount</label>
                                <input type="number" name="ranges[{{ $idx }}][min]" value="{{ $r['min'] }}" class="w-full bg-white border border-slate-300 text-slate-900 text-sm rounded-lg px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" placeholder="Min">
                            </div>
                            <div class="md:col-span-4">
                                <label class="block text-[10px] text-slate-500 uppercase tracking-wider mb-1 px-1">Max Amount</label>
                                <input type="number" name="ranges[{{ $idx }}][max]" value="{{ $r['max'] }}" class="w-full bg-white border border-slate-300 text-slate-900 text-sm rounded-lg px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" placeholder="Max">
                            </div>
                            <div class="md:col-span-4 flex items-end justify-between h-full pb-2">
                                <label class="flex items-center cursor-pointer select-none">
                                    <input type="hidden" name="ranges[{{ $idx }}][hide]" value="0">
                                    <input type="checkbox" name="ranges[{{ $idx }}][hide]" value="1" {{ $r['hide'] ? 'checked' : '' }} class="w-4 h-4 rounded border-slate-300 text-blue-600 bg-white focus:ring-blue-500">
                                    <span class="ml-2 text-sm text-slate-700 group-hover:text-slate-900 transition-colors">Active</span>
                                </label>
                                <button type="button" class="text-xs bg-white border border-slate-300 text-slate-700 px-2.5 py-1 rounded-md hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition" onclick="openRangeBills({{ $r['min'] }}, {{ $r['max'] }})" title="View bills in this range">
                                    <i class="fa-solid fa-receipt mr-1"></i>{{ $rangeCounts[$idx] ?? 0 }} bills
                                </button>
                ...
                @if(count($config['hidden_ranges']) === 0)
                    <div id="empty-ranges" class="text-center py-8 text-slate-500 text-sm">
                        No hidden ranges configured.
                    </div>
                @endif
            </div>
    ...
    <div id="rangeModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute inset-x-0 top-10 mx-auto max-w-3xl bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50">
                <h4 class="text-slate-800 font-semibold"><i class="fa-solid fa-list mr-2 text-blue-600"></i>Bills in Range</h4>
    ...
    <script>
        function addRange(){
            ...
            const idx = container.querySelectorAll('.grid').length;
            
            // Remove empty message if exists
            const emptyMsg = document.getElementById('empty-ranges');
            if(emptyMsg) emptyMsg.remove();

            const wrapper = document.createElement('div');
            wrapper.className = 'grid grid-cols-1 md:grid-cols-12 gap-3 items-center bg-slate-50 p-3 rounded-lg border border-slate-200 hover:border-slate-300 transition-colors group';
            wrapper.innerHTML = `
                <div class="md:col-span-4">
                    <label class="block text-[10px] text-slate-500 uppercase tracking-wider mb-1 px-1">Min Amount</label>
                    <input type="number" name="ranges[${idx}][min]" class="w-full bg-white border border-slate-300 text-slate-900 text-sm rounded-lg px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" placeholder="Min">
                </div>
                <div class="md:col-span-4">
                    <label class="block text-[10px] text-slate-500 uppercase tracking-wider mb-1 px-1">Max Amount</label>
                    <input type="number" name="ranges[${idx}][max]" class="w-full bg-white border border-slate-300 text-slate-900 text-sm rounded-lg px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" placeholder="Max">
                </div>
                <div class="md:col-span-4 flex items-end justify-between h-full pb-2">
                    <label class="flex items-center cursor-pointer select-none">
                        <input type="hidden" name="ranges[${idx}][hide]" value="0">
                        <input type="checkbox" name="ranges[${idx}][hide]" value="1" class="w-4 h-4 rounded border-slate-300 text-blue-600 bg-white focus:ring-blue-500">
                        <span class="ml-2 text-sm text-slate-700 group-hover:text-slate-900 transition-colors">Active</span>
                    </label>
                    <button type="button" class="text-slate-500 hover:text-red-600 p-1.5 rounded-md hover:bg-red-50 transition-all" onclick="removeRange(this)" title="Remove Range">
                        <i class="fas fa-trash-alt"></i>
        ...
        async function openRangeBills(minAmount, maxAmount){
            ...
            try {
                const params = new URLSearchParams({min: String(minAmount), max: String(maxAmount)});
